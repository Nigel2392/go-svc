...
func (si *serviceInstaller) Install() error {
	...
	if err != nil {
		return fmt.Errorf("Install.mgr.CreateService(): failed: %s", err)
	}
	defer s.Close()
	...
	if err != nil {
		s.Delete()
		return fmt.Errorf("Install.eventlog.SetupEventLogSource(): failed: %s", err)
	}
...
func (si *serviceInstaller) Remove() error {
	...
	err = s.Delete()
	if err != nil {
		return fmt.Errorf("Remove.service.Delete(): failed: %s", err)
	}

	err = eventlog.Remove(serviceName)
	if err != nil {
		return fmt.Errorf("Remove.eventlog.Remove(): failed: %s", err)
	}
